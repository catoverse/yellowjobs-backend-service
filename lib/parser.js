...
const parseJobType = (text) => {
  ...
  keywordMatch(text, (word) => {
    if (["intern", "interns", "internship", "apprentice", "apprenticeship"].includes(word)) {
      type = "internship";
    } else if (["freelance", "temporary", "budget", "project", "contract", "gig", "gigs", "parttime"].includes(word)) {
      type = "freelance";
    } else if (word == "fulltime") {
      type = "fulltime";
    } else if (["freshers", "noexperience", "fresher"].includes(word)) {
      type = "freshers";
    } else {
