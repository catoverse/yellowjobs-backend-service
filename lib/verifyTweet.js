...
const verifyTweet = (tweet) => {
  ...
  if(tweet.location) return 0;

  if(r == 2) return 1;
  if(r == 0) return 0;

  if(tweet.text.length < 75) return 0;
  
  if(tweet.urls.length == 0 && tweet.email.length == 0 && tweet.text.toLowerCase().search(/dm|comment|contact|apply/) == -1) return 0;
  ...
  //   // delete tweets if it's a fulltime non-tech and non-design job
...
module.exports.verifyTweetText = verifyTweetText;